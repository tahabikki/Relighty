#!/usr/bin/env python
"""
Shadow removal inference using mask_bg_remove approach.
...
def remove_shadow(model, mask_gen, img_bgr, device, img_size=256, keep_bg=False):
    ...
    if face_pts is not None:
        combined_mask = create_face_neck_mask(resized, face_pts, img_size, extend_forehead=True, forehead_extension=0.08)
    else:
    ...
    if keep_bg:
        mask_bool_orig = combined_mask > 0.3
        bg_mask = 1.0 - combined_mask
        bg_mask = cv2.GaussianBlur(bg_mask, (21, 21), 0)
        
        result_f = result_img.astype(np.float32)
        orig_f = orig_bgr.astype(np.float32)
        bg_f = orig_f * bg_mask[:, :, np.newaxis]
        
        final = result_f * combined_mask[:, :, np.newaxis] + bg_f
