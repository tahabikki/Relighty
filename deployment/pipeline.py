"""
End-to-end shadow removal pipeline using mask_bg_remove approach.
...
class ShadowRemovalPipeline:
    """
    End-to-end pipeline using mask_bg_remove approach.
    ...
    def __init__(
        ...
        self.mask_gen = FaceMaskGenerator(self.image_size)
    ...
    def process_image(self, image: np.ndarray) -> np.ndarray:
        ...
        face_mask, face_pts = self.mask_gen.detect(rgb)

        if face_pts is not None:
            combined_mask = create_face_neck_mask(
                resized, face_pts, self.image_size,
                extend_forehead=True
            )
        else:
